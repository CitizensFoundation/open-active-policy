define(["exports","../oap-app.js"],function(_exports,_oapApp){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.ypTruncateBehavior=_exports.ypRemoveClassBehavior=_exports.ypNumberFormatBehavior=_exports.ypMediaFormatsBehavior=_exports.ypLocalizationBridgeBehavior=_exports.ypIronListBehavior=_exports.YpPostBehavior=_exports.$ypTruncateBehavior=_exports.$ypRemoveClassBehavior=_exports.$ypPostBehaviors=_exports.$ypNumberFormatBehavior=_exports.$ypMediaFormatsBehavior=_exports.$ypLocalizationBridgeBehavior=_exports.$ypIronListBehavior=_exports.$esmDefault=_exports.$esm=void 0;(0,_oapApp.Polymer)({is:"iron-request",hostAttributes:{hidden:!0},properties:{/**
...
if(null===str||str===void 0||!str.toString){return""}return encodeURIComponent(str.toString().replace(/\r?\n/g,"\r\n")).replace(/%20/g,"+")},/**
   ...
   */_updateStatus:function(){this._setStatus(this.xhr.status);this._setStatusText(this.xhr.statusText===void 0?"":this.xhr.statusText)}});(0,_oapApp.Polymer)({is:"iron-ajax",/**
...
`;template.setAttribute("style","display: none;");document.head.appendChild(template.content);var IOS=navigator.userAgent.match(/iP(?:hone|ad;(?: U;)? CPU) OS (\d+)/),IOS_TOUCH_SCROLLING=IOS&&8<=IOS[1],DEFAULT_PHYSICAL_COUNT=3,HIDDEN_Y="-10000px",SECRET_TABINDEX=-100;/**
                            ...
                            */(0,_oapApp.Polymer)({_template:_oapApp.html`
    <style>
      :host {
        display: block;
      }
...
`,is:"iron-list",properties:{/**
     ...
     */scrollOffset:{type:Number,value:0}},observers:["_itemsChanged(items.*)","_selectionEnabledChanged(selectionEnabled)","_multiSelectionChanged(multiSelection)","_setOverflow(scrollTarget, scrollOffset)"],behaviors:[_oapApp.Templatizer,_oapApp.IronResizableBehavior,_oapApp.IronScrollTargetBehavior,_oapApp.OptionalMutableDataBehavior],/**
   ...
   */get _itemsParent(){return(0,_oapApp.dom)((0,_oapApp.dom)(this._userTemplate).parentNode)},/**
...
this._lastVisibleIndexVal=null;this._firstVisibleIndexVal=null;this._debounce("_render",this._render,_oapApp.animationFrame)},/**
...
}else if(!this._isClientFull()){this._debounce("_increasePoolIfNeeded",this._increasePoolIfNeeded.bind(this,nextIncrease),_oapApp.microTask)}else if(this._physicalSize<this._optPhysicalSize){// Yield and increase the pool during idle time until the physical size is
// optimal.
this._debounce("_increasePoolIfNeeded",this._increasePoolIfNeeded.bind(this,this._clamp(Math.round(50/this._templateCost),1,nextIncrease)),_oapApp.idlePeriod)}},/**
...
this.updateViewportBoundaries();this._increasePoolIfNeeded(DEFAULT_PHYSICAL_COUNT)}},/**
   ...
   */_ensureTemplatized:function(){if(this.ctor){return}this._userTemplate=this.queryEffectiveChildren("template");if(!this._userTemplate){console.warn("iron-list requires a template to be provided in light-dom")}var instanceProps={__key__:!0};instanceProps[this.as]=!0;instanceProps[this.indexAs]=!0;instanceProps[this.selectedAs]=!0;instanceProps.tabIndex=!0;this._instanceProps=instanceProps;this.templatize(this._userTemplate,this.mutableData)},_gridChanged:function(newGrid,oldGrid){if("undefined"===typeof oldGrid)return;this.notifyResize();(0,_oapApp.flush)();newGrid&&this._updateGridMetrics()},/**
   * Called when the items have changed. That is, reassignments
   * to `items`, splices or updates to a single item.
   */_itemsChanged:function(change){if("items"===change.path){this._virtualStart=0;this._physicalTop=0;this._virtualCount=this.items?this.items.length:0;this._physicalIndexForKey={};this._firstVisibleIndexVal=null;this._lastVisibleIndexVal=null;this._physicalCount=this._physicalCount||0;this._physicalItems=this._physicalItems||[];this._physicalSizes=this._physicalSizes||[];this._physicalStart=0;if(this._scrollTop>this._scrollOffset){this._resetScrollPosition(0)}this._removeFocusedItem();this._debounce("_render",this._render,_oapApp.animationFrame)}else if("items.splices"===change.path){this._adjustVirtualIndex(change.value.indexSplices);this._virtualCount=this.items?this.items.length:0;// Only blur if at least one item is added or removed.
...
var affectedIndexRendered=change.value.indexSplices.some(function(splice){return splice.index+splice.addedCount>=this._virtualStart&&splice.index<=this._virtualEnd},this);if(!this._isClientFull()||affectedIndexRendered){this._debounce("_render",this._render,_oapApp.animationFrame)}}else if("items.length"!==change.path){this._forwardItemPath(change.path,change.value)}},_forwardItemPath:function(path,value){path=path.slice(6);// 'items.'.length == 6
...
(0,_oapApp.flush)();var newPhysicalSize=0,oldPhysicalSize=0,prevAvgCount=this._physicalAverageCount,prevPhysicalAvg=this._physicalAverage;this._iterateItems(function(pidx,vidx){oldPhysicalSize+=this._physicalSizes[pidx];this._physicalSizes[pidx]=this._physicalItems[pidx].offsetHeight;newPhysicalSize+=this._physicalSizes[pidx];this._physicalAverageCount+=this._physicalSizes[pidx]?1:0},itemSet);if(this.grid){this._updateGridMetrics();this._physicalSize=Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight}else{oldPhysicalSize=1===this._itemsPerRow?oldPhysicalSize:Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight;this._physicalSize=this._physicalSize+newPhysicalSize-oldPhysicalSize;this._itemsPerRow=1}// Update the average if it measured something.
...
if(forceUpdate||Math.abs(this._estScrollHeight-this._scrollHeight)>=this._viewportHeight){this.$.items.style.height=this._estScrollHeight+"px";this._scrollHeight=this._estScrollHeight}},/**
   ...
   */scrollToIndex:function(idx){if("number"!==typeof idx||0>idx||idx>this.items.length-1){return}(0,_oapApp.flush)();// Items should have been rendered prior scrolling to an index.
...
this.toggleScrollListener(!1)}},_oapApp.animationFrame)},/**
   ...
   */_selectionHandler:function(e){var model=this.modelForElement(e.target);if(!model){return}var modelTabIndex,activeElTabIndex,target=(0,_oapApp.dom)(e).path[0],activeEl=this._getActiveElement(),physicalItem=this._physicalItems[this._getPhysicalIndex(model[this.indexAs])];// Safari does not focus certain form controls via mouse
...
if(!focusable){focusable=(0,_oapApp.dom)(physicalItem).querySelector("[tabindex=\""+SECRET_TABINDEX+"\"]")}// restore the tab index
...
<div id="radioLabel"><slot></slot></div>`;template$1.setAttribute("strip-whitespace","");/**
                                                 ...
                                                 @demo demo/index.html
                                                 */(0,_oapApp.Polymer)({_template:template$1,is:"paper-radio-button",behaviors:[_oapApp.PaperCheckedElementBehavior],hostAttributes:{role:"radio","aria-checked":!1,tabindex:0},properties:{/**
...
(0,_oapApp.afterNextRender)(this,function(){var inkSize=this.getComputedStyleValue("--calculated-paper-radio-button-ink-size").trim();// If unset, compute and set the default `--paper-radio-button-ink-size`.
...
// centers align.
if(defaultInkSize%2!==size%2){defaultInkSize++}this.updateStyles({"--paper-radio-button-ink-size":defaultInkSize+"px"})}})}});(0,_oapApp.Polymer)({_template:_oapApp.html`
    <style>
      :host {
        display: inline-block;
      }
    ...
    <slot></slot>
`,is:"paper-radio-group",behaviors:[_oapApp.IronMenubarBehavior],/** @private */hostAttributes:{role:"radiogroup"},properties:{/**
...
`,is:"yp-post",behaviors:[ypMediaFormatsBehavior,ypNumberFormatBehavior,ypTruncateBehavior,ypLocalizationBridgeBehavior],properties:{idRoute:Object,tabRoute:Object,idRouteData:Object,tabRouteData:Object,postId:{type:Number,value:null,observer:"_postIdChanged"},host:String,post:{type:Object,value:null,notify:!0,observer:"_postChanged"},selectedTab:{type:String,value:"debate",observer:"_selectedTabChanged"},small:{type:Boolean},method:{type:String},mapActive:{type:Boolean,value:!1},wideWidth:{type:Boolean,value:!1},createFabIcon:{type:String,value:"lightbulb-outline"},disableNewPosts:{type:Boolean,value:!1},scrollToPointId:{type:String,value:null},locationHidden:{type:Boolean,value:!1}},observers:["_routeIdChanged(idRouteData.id)","_routeTabChanged(tabRouteData.tabName)"],_routeIdChanged:function(newId){if(newId){this.set("postId",newId)}},_routeTabChanged:function(newTabName){if(newTabName&&!this._isNumber(newTabName)){this.set("selectedTab",newTabName)}else if(newTabName&&this._isNumber(newTabName)){this.set("scrollToPointId",newTabName);this.set("selectedTab","debate")}},_isNumber:function(n){return!isNaN(parseFloat(n))&&isFinite(n)},_selectedTabChanged:function(tabName){if(this.post){this.redirectTo("/post/"+this.post.id+"/"+tabName)}if("location"==tabName){this.set("mapActive",!0)}else{this.set("mapActive",!1)}if(tabName&&window.appGlobals){window.appGlobals.activity("open","post_tab_"+tabName,"",{id:this.postId,modelType:"post"})}this.async(function(){var news=this.$$("#postNews");if(news){news.fireResize()}},300)},listeners:{"yp-debate-info":"_updateDebateInfo","yp-post-image-count":"_updatePostImageCount"},_updatePostImageCount:function(event,imageCount){var tabCounter=this.$$("#tabCountPhotos");if(tabCounter){tabCounter.innerHTML=this.formatNumber(imageCount)}},_close:function(){window.history.back()},_updateDebateInfo:function(event,detail){var tabCounter=this.$$("#tabCountDebate");if(tabCounter){tabCounter.innerHTML=this.formatNumber(detail.count)}if(detail.firstPoint){this.$.postCard.updateDescriptionIfEmpty(detail.firstPoint.content)}},_mainContainerClasses:function(small){if(small){return"layout horizontal wrap"}else{return"layout horizontal center-center"}},_headerClasses:function(small){if(small){return"layout vertical postHeader wrap"}else{return"layout horizontal postHeader"}},postName:function(post){if(post&&post.name){return this.truncate(this.trim(post.name),200)}else if(post){return post.short_name}},postDescription:function(post){if(post&&post.description){return this.truncate(this.trim(post.description),1e4,"...")}else{return""}},_refreshAjax:function(){this.$.ajax.generateRequest()},_postChanged:function(newValue,oldValue){},_postIdChanged:function(postId){var cachedItem=null;if(cachedItem){this._setupAjaxUrl();this._handleIncomingPostResponse(null,{response:cachedItem});console.log("Got post from item cache")}else if(!1){this._setupAjaxUrl();this._handleIncomingPostResponse(null,{response:window.appGlobals.getPostFromCache(postId),fromCache:!0});console.log("Got post from cache possibly from recommendations")}else{console.log("Got post from server not cache");this.set("post",null);if(postId){this._getPost();this.set("selectedTab","debate")}}},_setupAjaxUrl:function(){if(this.host){this.$.ajax.url=this.host+"/api/posts/"+this.postId}else{this.$.ajax.url="/api/posts/"+this.postId}},_pagesResponse:function(event,detail){this.fire("yp-set-pages",detail.response)},_getPost:function(){this._setupAjaxUrl();this.$$("#ajax").retryMethodAfter401Login=this._getPost.bind(this);this.$.ajax.generateRequest()},_handleIncomingPostResponse:function(event,detail){this.set("post",detail.response);this.refresh();if(this.post.Group.configuration&&this.post.Group.configuration.canAddNewPosts!=void 0){if(!0===this.post.Group.configuration.canAddNewPosts){this.set("disableNewPosts",!1)}else{this.set("disableNewPosts",!0)}}else{this.set("disableNewPosts",!1)}},_processRecommendation:function(recommendedPost){if(recommendedPost&&this.post){var postName=recommendedPost.name;if(this.wideWidth){postName=this.truncate(postName,60)}else{postName=this.truncate(postName,30)}this.fire("yp-set-next-post",{currentPostId:this.post.id,goForwardToPostId:recommendedPost.id,goForwardPostName:postName})}else{this.fire("yp-set-next-post",{currentPostId:this.post.id,goForwardToPostId:null,goForwardPostName:null});console.log("Not recommended post")}},refresh:function(){if(this.post){if(null!=this.post.Group.theme_id||this.post.Group.configuration&&null!=this.post.Group.configuration.themeOverrideColorPrimary){}else if(this.post.Group.Community&&(null!=this.post.Group.Community.theme_id||this.post.Group.Community.configuration&&this.post.Group.Community.configuration.themeOverrideColorPrimary)){}if(!this.post.Group.Community){console.error("No community!");debugger}/*
        ...
        */}},setupTopHeaderImage:function(image){var url="url("+this.getImageFormatUrl(image,0)+")";this.updateStyles({"--top-area-background-image":url})},computeUrl:function(post_id){return"/api/posts/"+post_id}})});